...
def find_app_by_string(module: ModuleType, app_name: str) -> Quart:
    ...
    raise NoAppException(
        f"A valid Quart application was not obtained from "
    ...
    def load_app(self) -> Quart:
        ...
        if self.create_app is not None:
            ...
            if self.app_import_path:
                path, name = (
                    re.split(
                        pattern=r":(?![\\/])", string=self.app_import_path, maxsplit=1
                    )
                    + [None]
                )[:2]
            ...
            async with __ctx.ensure_object(ScriptInfo).load_app().app_context():
                try:
                    ...
                    if (
                        ...
                        click.echo(
                            "The appcontext cannot be used with a command that runs an "
                            "event loop. See quart#361 for more details"
                        )
                    raise
...
@click.option(
    "--insecure-port", default=None, help="The port to bind to for the insecure host."
)
@click.option(
    "--certfile",
    "--cert",
